...
object SalaryPredictorService {
  ...
  def predictSalary(programmers: Seq[SalaryPredictor.ProgrammerFeatures]): ZIO[Has[SalaryPredictor], PredictorError, Seq[EarningsPrediction]] = {
    val workHoursInMonth = 160
    SalaryPredictor.predict(programmers)
